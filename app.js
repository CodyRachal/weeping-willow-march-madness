console.log("Weeping Willow bracket app loaded ✅");
...
function renderTop3() {
  ...
  if (!top.length) {
    const el = $("top3");
    if (el) el.innerHTML = `<div class="small">No paid entries scored yet (or results not updated).</div>`;
    return;
  }
...
function renderRules() {
  ...
  // Front-page Players/Pot line
  const metaLine = document.getElementById("metaLine");
  if (metaLine) metaLine.textContent = `Players: ${paidCount} • Pot: $${pot}`;
...
function buildPickUI(name, unlocked) {
  ...
  $("exportBtn").onclick = () => {
    const payload = { name, tiebreaker: draft.tiebreaker, picks: draft.picks };
...
async function init() {
  ...
  try {
    ...
    const el = $("top3");
    if (el) el.innerHTML = `<div class="small">Couldn’t load data files. Check file paths and hosting.</div>`;
  }
}
